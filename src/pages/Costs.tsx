...
export function Costs() {
  ...
  const activeSelectedServices = useMemo(
    () => savedProposal ? selectedServices.filter((serviceId) => planServiceIds.includes(serviceId)) : [],
    [planServiceIds, savedProposal, selectedServices],
  )
  ...
  const costAreaData = calculatedItems.map((item) => ({
    service: item.service.replace('Amazon ', ''),
    cost: Math.round(item.monthly),
  }))
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid min-w-0 grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,.95fr)]">
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_12px_30px_rgba(15,23,42,0.05)] sm:p-6">
          ...
          <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50/50 p-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold text-slate-700">Servicios facturables del plan</p><p className="mt-0.5 text-[11px] leading-5 text-slate-500">Elige qué servicios del plan entran en la estimación actual.</p></div><span className="text-[11px] font-bold text-blue-700">{activeSelectedServices.length} de {plannedCostItems.length}</span></div><div className="mt-3 flex flex-wrap gap-2">{plannedCostItems.length > 0 ? plannedCostItems.map((item) => { const selected = activeSelectedServices.includes(item.serviceId); return <Button type="button" key={item.serviceId} size="sm" variant={selected ? 'default' : 'outline'} onClick={() => toggleService(item.serviceId)} aria-pressed={selected} className={`rounded-full text-[11px] font-bold ${selected ? 'bg-blue-600 text-white hover:bg-blue-700' : 'border-slate-300 bg-white text-slate-500 hover:border-blue-300 hover:text-blue-700'}`}>{selected && <Icon name="check" className="text-[14px]" />}{item.service}</Button> }) : <span className="text-xs text-slate-500">La planificación solo contiene servicios sin consumo directo estimable.</span>}</div><p className="mt-3 border-t border-blue-100 pt-3 text-[10px] leading-4 text-slate-500">Solo puedes seleccionar servicios definidos en la planificación. IAM y VPC se muestran en la arquitectura, pero no tienen un cargo base calculable sin componentes asociados.</p></div>

          <div className="mt-4 divide-y divide-slate-100 rounded-xl border border-slate-200">
            {calculatedItems.length > 0 ? calculatedItems.map((item) => (
              <div key={item.serviceId} className="grid gap-3 px-3 py-4 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center sm:px-4">
                ...
                <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-500"><div><span className="block text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Cantidad</span><div className="mt-1 inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 p-1"><Button type="button" variant="ghost" size="icon-sm" onClick={() => changeQuantity(item.serviceId, -1)} disabled={item.quantity === 0} aria-label={`Reducir cantidad de ${item.service}`} className="text-slate-500 hover:bg-white hover:text-slate-800"><Icon name="remove" className="text-[16px]" /></Button><output aria-label={`Cantidad actual de ${item.service}`} className="w-9 text-center text-sm font-extrabold text-slate-800">{item.quantity}</output><Button type="button" variant="ghost" size="icon-sm" onClick={() => changeQuantity(item.serviceId, 1)} disabled={item.quantity === 20} aria-label={`Aumentar cantidad de ${item.service}`} className="bg-white text-blue-600 shadow-sm hover:bg-blue-50"><Icon name="add" className="text-[16px]" /></Button></div></div><div><span className="block text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Horas</span><strong className="mt-1 block text-xs text-slate-700">{item.hours} h/mes</strong></div><div><span className="block text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Tarifa</span><strong className="mt-1 block text-xs text-slate-700">{formatCurrency(item.unitHourlyCost, 2)}/h</strong></div></div>
        ...
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_12px_30px_rgba(15,23,42,0.05)] sm:p-6">
          ...
          {calculatedItems.length > 0 ? (
            <>
              ...
              <div className="mt-5 border-t border-slate-100 pt-5"><div className="flex items-center justify-between gap-3"><div><h4 className="text-xs font-extrabold uppercase tracking-[0.1em] text-slate-500">Costo por servicio</h4><p className="mt-1 text-[11px] text-slate-400">Distribución calculada con los servicios seleccionados del plan.</p></div><span className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-700">Interactivo</span></div><div className="mt-2 h-[190px] w-full"><ResponsiveContainer width="100%" height="100%"><AreaChart data={costAreaData} margin={{ top: 10, right: 8, left: -18, bottom: 0 }}><defs><linearGradient id="costAreaFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#2563eb" stopOpacity="0.3" /><stop offset="100%" stopColor="#2563eb" stopOpacity="0.03" /></linearGradient></defs><CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3" /><XAxis dataKey="service" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10 }} tickFormatter={(value) => `$${value}`} /><Tooltip cursor={{ stroke: '#93c5fd', strokeDasharray: '4 4' }} formatter={(value) => [formatCurrency(Number(value ?? 0)), 'Mensual']} /><Area type="monotone" dataKey="cost" stroke="#2563eb" strokeWidth={2.5} fill="url(#costAreaFill)" activeDot={{ r: 5, fill: '#2563eb', stroke: '#fff', strokeWidth: 2 }} isAnimationActive={false} /></AreaChart></ResponsiveContainer></div></div>
            </>
          ) : (
