...
const regionConnections = [
  { id: 'us-eu', label: 'US East ↔ EU Ireland', detail: 'Replicación activa y failover disponible.', path: 'M 225 172 Q 330 92 432 141' },
  { id: 'us-sa', label: 'US East ↔ São Paulo', detail: 'Ruta alternativa para continuidad operativa.', path: 'M 225 172 Q 250 244 324 304' },
  { id: 'eu-sa', label: 'EU Ireland ↔ São Paulo', detail: 'Conexión secundaria entre regiones.', path: 'M 432 141 Q 390 232 324 304' },
]

type GeoJsonPosition = [number, number]
type GeoJsonGeometry = { type: 'Polygon'; coordinates: GeoJsonPosition[][] } | { type: 'MultiPolygon'; coordinates: GeoJsonPosition[][][] }
type GeoJsonFeature = { type: 'Feature'; properties: { country: string }; geometry: GeoJsonGeometry }
const regionCountries: Record<string, string> = { 'us-east-1': 'usa', 'eu-west-1': 'ireland', 'sa-east-1': 'brazil' }

function projectCoordinate([longitude, latitude]: GeoJsonPosition) {
  return `${((longitude + 180) / 360) * 900} ${((90 - latitude) / 180) * 440}`
}

function ringToPath(ring: GeoJsonPosition[]) {
  if (ring.length === 0) return ''

  const commands = [`M ${projectCoordinate(ring[0])}`]
  let crossesDateLine = false

  for (let index = 1; index < ring.length; index += 1) {
    const previous = ring[index - 1]
    const current = ring[index]
    const crossesSegment = Math.abs(previous[0] - current[0]) > 180
    crossesDateLine ||= crossesSegment
    commands.push(`${crossesSegment ? 'M' : 'L'} ${projectCoordinate(current)}`)
  }

  crossesDateLine ||= Math.abs(ring[ring.length - 1][0] - ring[0][0]) > 180
  return `${commands.join(' ')}${crossesDateLine ? '' : ' Z'}`
}

function featureToPath(feature: GeoJsonFeature) {
  return feature.geometry.type === 'Polygon'
    ? feature.geometry.coordinates.map(ringToPath).join(' ')
    : feature.geometry.coordinates.flatMap((polygon) => polygon.map(ringToPath)).join(' ')
}
...
function WorldMap({ selectedCode, onSelect }: { selectedCode: string; onSelect: (code: string) => void }) {
  const [worldFeatures, setWorldFeatures] = useState<GeoJsonFeature[]>([])
  const [selectedConnectionId, setSelectedConnectionId] = useState(regionConnections[0].id)
  const selectedCountry = regionCountries[selectedCode]
  const selectedConnection = regionConnections.find((connection) => connection.id === selectedConnectionId) ?? regionConnections[0]

  useEffect(() => {
    let active = true
    fetch(`${import.meta.env.BASE_URL}world-boundaries.json`)
      .then((response) => {
        if (!response.ok) throw new Error('No se pudo cargar el mapa mundial')
        return response.json() as Promise<{ features: GeoJsonFeature[] }>
      })
      .then((data) => {
        if (active) setWorldFeatures(data.features)
      })
      .catch(() => {
        if (active) setWorldFeatures([])
      })
  ...
  return (
    <section className="h-fit overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04),0_12px_30px_rgba(15,23,42,0.05)]">
      ...
      <div className="relative min-h-[330px] overflow-hidden bg-[#f8fafc] sm:min-h-[360px]">
        <svg viewBox="0 0 900 440" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-label="Mapa mundial con fronteras GeoJSON">
          <defs><linearGradient id="ocean" x1="0" x2="1" y1="0" y2="1"><stop stopColor="#f8fafc" /><stop offset="1" stopColor="#eff6ff" /></linearGradient></defs>
          <rect width="900" height="440" fill="url(#ocean)" />
          <g strokeLinejoin="round">
            {worldFeatures.map((feature, index) => <path key={`${feature.properties.country}-${index}`} d={featureToPath(feature)} fill={feature.properties.country === selectedCountry ? '#bfdbfe' : '#dbe4ef'} fillOpacity={feature.properties.country === selectedCountry ? '1' : '0.9'} stroke={feature.properties.country === selectedCountry ? '#2563eb' : '#cbd5e1'} strokeOpacity={feature.properties.country === selectedCountry ? '1' : '0.65'} strokeWidth={feature.properties.country === selectedCountry ? '1' : '0.45'} vectorEffect="non-scaling-stroke" />)}
          </g>
          <g fill="none" strokeLinecap="round">
            {regionConnections.map((connection) => <path key={connection.id} d={connection.path} stroke="#bfdbfe" strokeWidth="14" opacity="0" role="button" tabIndex={0} aria-label={`Seleccionar conexión ${connection.label}`} onClick={() => setSelectedConnectionId(connection.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedConnectionId(connection.id) }} />)}
            {regionConnections.map((connection) => <path key={`${connection.id}-visible`} d={connection.path} stroke={connection.id === selectedConnection.id ? '#2563eb' : '#60a5fa'} strokeDasharray="5 6" strokeWidth={connection.id === selectedConnection.id ? '2.5' : '1.8'} opacity={connection.id === selectedConnection.id ? '1' : '.72'} />)}
          </g>
          {worldFeatures.length === 0 && <text x="450" y="225" textAnchor="middle" fill="#94a3b8" fontSize="13" fontWeight="600">Cargando mapa mundial…</text>}
        </svg>
        <div className="absolute right-3 top-3 w-[210px] rounded-xl border border-blue-100 bg-white/95 p-3 shadow-sm backdrop-blur"><div className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.08em] text-blue-700"><span className="w-4 border-t-2 border-dashed border-blue-500" />Conexión seleccionada</div><p className="mt-1.5 text-xs font-bold text-slate-800">{selectedConnection.label}</p><p className="mt-1 text-[10px] leading-4 text-slate-500">{selectedConnection.detail}</p><span className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700"><span className="size-1.5 rounded-full bg-emerald-500" />Failover disponible</span></div>
        {regions.map((region) => {
        ...
        <div className="absolute bottom-4 left-4 flex flex-wrap gap-3 rounded-xl border border-white/80 bg-white/90 px-3 py-2 text-[10px] font-bold text-slate-500 shadow-sm backdrop-blur"><span className="inline-flex items-center gap-1.5"><i className="size-2 rounded-full bg-emerald-500" />Operativa</span><span className="inline-flex items-center gap-1.5"><i className="size-2 rounded-full bg-amber-500" />Revisión</span><span className="inline-flex items-center gap-1.5"><i className="w-4 border-t-2 border-dashed border-blue-400" />Failover</span><span>{regions.length} regiones desplegadas</span></div>
      </div>
    </section>
  )
}
...
function InfrastructureConsole({ selectedRegion, onSelectRegion }: { selectedRegion: Region; onSelectRegion: (code: string) => void }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04),0_12px_30px_rgba(15,23,42,0.05)]"><div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50/80 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="flex size-9 items-center justify-center rounded-lg bg-orange-100 text-orange-700"><Icon name="monitoring" className="text-[18px]" /></div><div><div className="flex items-center gap-2"><p className="text-xs font-extrabold text-slate-800">Consola de producción</p><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] font-extrabold text-emerald-700">OPERATIVA</span></div><p className="mt-0.5 text-[10px] text-slate-500">Cuenta CloudOps · actualización automática</p></div></div><div className="flex items-center gap-2"><span className="text-[10px] font-bold text-slate-500">Región</span><Select value={selectedRegion.code} onValueChange={onSelectRegion}><SelectTrigger aria-label="Seleccionar región de consola" className="h-auto min-w-[190px] rounded-lg border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700"><SelectValue /></SelectTrigger><SelectContent align="end">{regions.map((region) => <SelectItem key={region.code} value={region.code}>{region.code} · {region.location}</SelectItem>)}</SelectContent></Select></div></div>
    <Tabs value={view} onValueChange={(value) => setView(value as ConsoleView)} className="gap-0">
      <TabsList className="mx-2 my-2 flex w-[calc(100%-1rem)] rounded-xl bg-slate-100 p-1">
        {([{ id: 'summary', label: 'Resumen', icon: 'dashboard' }, { id: 'resources', label: 'Recursos', icon: 'dns' }, { id: 'monitoring', label: 'Monitoreo en vivo', icon: 'monitoring' }] as Array<{ id: ConsoleView; label: string; icon: string }>).map((item) => <TabsTrigger key={item.id} value={item.id} className="flex-1 rounded-lg px-2 py-2 text-[11px] font-bold sm:text-xs"><Icon name={item.icon} className="text-[16px]" />{item.label}</TabsTrigger>)}
      </TabsList>
    </Tabs>
    {view === 'summary' && <div className="p-4 sm:p-5"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-slate-200 p-3"><span className="text-[9px] font-extrabold uppercase tracking-[0.1em] text-slate-400">Salud</span><strong className="mt-1.5 block text-xl text-slate-800">{resources.length}/{resources.length}</strong><p className="mt-1 text-[11px] text-emerald-700">Recursos disponibles</p></div><div className="rounded-xl border border-slate-200 p-3"><span className="text-[9px] font-extrabold uppercase tracking-[0.1em] text-slate-400">Disponibilidad</span><strong className="mt-1.5 block text-xl text-slate-800">{selectedRegion.availability}</strong><p className="mt-1 text-[11px] text-slate-500">Últimos 30 días</p></div><div className="rounded-xl border border-slate-200 p-3"><span className="text-[9px] font-extrabold uppercase tracking-[0.1em] text-slate-400">Alarmas</span><strong className="mt-1.5 block text-xl text-amber-600">{selectedRegion.tone === 'warning' ? '1' : '0'}</strong><p className="mt-1 text-[11px] text-slate-500">Requieren atención</p></div></div><div className="mt-4"><div className="flex items-center justify-between"><h4 className="text-xs font-extrabold text-slate-800">Servicios desplegados</h4><Button type="button" variant="link" size="sm" onClick={() => setView('resources')} className="h-auto p-0 text-[11px] font-bold text-blue-600">Ver recursos</Button></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{selectedRegion.services.map((service) => <div key={service} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/70 p-3"><div className="flex size-8 items-center justify-center rounded-lg bg-white text-blue-600 shadow-sm"><Icon name={service === 'S3' ? 'database' : service === 'RDS' ? 'storage' : service === 'VPC' ? 'hub' : service === 'CloudFront' ? 'public' : 'dns'} className="text-[16px]" /></div><span className="min-w-0 flex-1 truncate text-xs font-bold text-slate-700">{serviceNames[service]}</span><span className="size-2 rounded-full bg-emerald-500" /></div>)}</div></div></div>}
    {view === 'resources' && <div><div className="flex flex-col gap-2 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="relative min-w-0 flex-1"><Icon name="search" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[16px] text-slate-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre, ID o servicio" className="h-auto rounded-lg border-slate-300 py-2.5 pl-9 pr-3 text-xs shadow-none focus-visible:border-orange-400 focus-visible:ring-orange-100" /></div><span className="text-[10px] font-bold text-slate-500">{filtered.length} recursos</span></div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-left"><thead className="border-b border-slate-200 bg-slate-50 text-[9px] font-extrabold uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-4 py-3">Recurso</th><th className="px-3 py-3">Servicio</th><th className="px-3 py-3">Zona</th><th className="px-3 py-3">Estado</th><th className="px-4 py-3 text-right">Métrica</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((resource) => <tr key={resource.id} className="text-xs hover:bg-orange-50/40"><td className="px-4 py-3"><div className="flex items-center gap-2"><div className="flex size-7 items-center justify-center rounded-md bg-orange-50 text-orange-600"><Icon name={resource.icon} className="text-[15px]" /></div><div><strong className="block text-slate-700">{resource.name}</strong><span className="font-mono text-[9px] text-slate-400">{resource.id}</span></div></div></td><td className="px-3 py-3 font-bold text-slate-600">{resource.service}</td><td className="px-3 py-3 text-slate-500">{resource.zone}</td><td className="px-3 py-3"><span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-emerald-700"><span className="size-1.5 rounded-full bg-emerald-500" />{resource.state}</span></td><td className="px-4 py-3 text-right font-bold text-slate-600">{resource.metric}</td></tr>)}</tbody></table></div></div>}
