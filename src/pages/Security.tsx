...
export function Security() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-emerald-600">Seguridad y gobierno</p><h2 className="mt-1 text-3xl font-bold tracking-[-0.045em] text-slate-800 sm:text-[32px]">Protección de la solución</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Revisa identidades, datos, cuentas y cumplimiento con prioridades claras.</p></div><Button type="button" size="lg" onClick={runReview} disabled={reviewing} className="w-full rounded-xl bg-blue-600 px-4 text-sm font-bold text-white hover:bg-blue-700 sm:w-auto"><Icon name="refresh" className={`text-[18px] ${reviewing ? 'animate-spin' : ''}`} />{reviewing ? 'Revisando controles...' : 'Ejecutar revisión'}</Button></header>
      ...
      <section><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h3 className="text-lg font-bold text-slate-800">Dominios de control</h3><p className="mt-1 text-sm text-slate-500">Selecciona un dominio para revisar su cobertura y siguiente acción.</p></div><div className="flex flex-wrap gap-2">{(['Todos', 'Correcto', 'Revisión', 'Problema'] as SecurityFilter[]).map((item) => <Button type="button" size="sm" variant={filter === item ? 'default' : 'outline'} key={item} onClick={() => setFilter(item)} className={filter === item ? 'rounded-full bg-blue-600 text-xs font-bold hover:bg-blue-700' : 'rounded-full border-slate-200 text-xs font-bold text-slate-500'}>{item}</Button>)}</div></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{visibleChecks.map((check) => <SecurityCard key={check.label} check={check} selected={selected.label === check.label} onSelect={() => setSelectedLabel(check.label)} />)}</div></section>
